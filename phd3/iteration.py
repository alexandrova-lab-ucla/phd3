...
class iteration:
    ...
    def __init__(self, controller, directory: str, root_dir: str, parameters: dict, iteration: int, cores=1, scratch="./"):
        ...
        logger.info(
            "+---------------------------------------------------------+")
        logger.info(
            f"|                     ITERATION      {self.iter_number:0>2d}                   |")
        logger.info(
            "+---------------------------------------------------------+")
        logger.info("")
        logger.info("")
    ...
    def continue_calculation(self):
        ...
        if self.next_step is None:
            ...
            logger.info(
                "+---------------------------------------------------------+")
            logger.info(
                f"|                 ITERATION      {self.iter_number:0>2d} COMPLETE              |")
            logger.info(
                "+---------------------------------------------------------+")
            logger.info("")
    ...
    def performDMD(self):
        ...
        if self.iter_number == 0:
            ...
            self.pdb_winner = utilities.load_pdb(
                self.parameters["last pdb"])

            self.pdb_winner.name = "movie_0000"
            self.pdb_winner.dmd_energy = 0.0
            ...
            if self.parameters["DMD CONVERGE"]:
                ...
                for i, d in enumerate(dirs):
                    if rm_dirs:
                        ...
                        logger.info(
                            f"Did not complete run {i+1}, removing directory and continuing")
                        shutil.rmtree(d)
                        # Want to remove any directories (if they exist) after this one! and start from here
                        rm_dirs = True
                        continue

                    echo_lines = dmd_simulation.get_echo_data(
                        os.path.join(d, self.parameters['dmd params']['Echo File']))
                    ...
                    if last_time < self.parameters['dmd params']['Time']:
                        ...
                        ave, stdev = dmd_simulation.get_average_potential_energy(
                            os.path.join(d, self.parameters["dmd params"]["Echo File"]))
                        dmd_average_energies.append([ave, stdev])
                        logger.info(
                            f"[Run {i+1:0>2d}] ==>> {ave:0<12.5f} ({stdev:0<9.5f})(finished)")
                ...
                if len(dmd_average_energies) >= 2:
                    ...
                    if self.dmd_converged(dmd_average_energies):
                        ...
                        logger.info(
                            f"Absolute delta in energy: {abs(dmd_average_energies[-1][0]-dmd_average_energies[-2][0]):.5f}")
                        converged = True
                        finished = True
                        self.final_dmd_average_energy = dmd_average_energies[-1] if dmd_average_energies[-1][
                            0] < dmd_average_energies[-2][0] else dmd_average_energies[-2]
                ...
                if os.path.isfile(self.parameters["dmd params"]["Echo File"]) and os.path.isfile(self.parameters["dmd params"]["Movie File"]):
                    ave, stdev = dmd_simulation.get_average_potential_energy(
                        self.parameters["dmd params"]["Echo File"])
                    logger.info(
                        f"[Run 01] ==>> {ave:0<12.5f} ({stdev:0<9.5f}) (finished)")
                    finished = True
        ...
        if not finished:
            ...
            if self.parameters["Equilibrate"]["Equilibrate On"] and not dmd_average_energies:
                if os.path.isdir("equilibrate") and os.path.isfile(f"equilibrate/{self.parameters['dmd params']['Echo File']}"):
                    ...
                    logger.info(
                        f"[Starting Temp.]   ==>> {self.parameters['Equilibrate']['Initial Temperature']}")
                    logger.info(
                        f"[Ending Temp.]     ==>> {self.parameters['dmd params']['Initial Temperature']}")
                    logger.info(
                        f"[Time]             ==>> {self.parameters['Equilibrate']['Time']}")
                    logger.info("")
                    ...
                    temp_params = copy.deepcopy(self.parameters["dmd params"])
                    ...
                    if temp_params["titr"]["titr on"]:
                        logger.warn("Turning titration off during equilibration!")
                        temp_params["titr"]["titr on"] = False

                    calc = dmd_simulation(
                        cores=self.cores, parameters=temp_params, run_dir=self.scratch)
                    self.stop = self.timer_went_off()
            ...
            if self.parameters["DMD CONVERGE"]:
                ...
                while not self.stop and self.dmd_steps <= self.parameters["MAX DMD STEPS"]:
                    ...
                    if os.path.isdir(f"dmdstep_{self.dmd_steps-1}"):
                        logger.debug(
                            f"Copying {self.dmd_steps-1} files to new dmd start")

                        shutil.copytree(
                            f"dmdstep_{self.dmd_steps-1}", f"dmdstep_{self.dmd_steps}")
                        logger.debug(
                            "Removing echo and movie file from new directory")
                        os.remove(os.path.join(
                            f"dmdstep_{self.dmd_steps}", self.parameters["dmd params"]["Echo File"]))
                        os.remove(os.path.join(
                            f"dmdstep_{self.dmd_steps}", self.parameters["dmd params"]["Movie File"]))

                    elif os.path.isdir("equilibrate") and os.path.isfile(f"equilibrate/{self.parameters['dmd params']['Restart File']}"):
                        logger.debug(
                            "Copying equilibrate files to new dmd start")
                        shutil.copytree(
                            "equilibrate", f"dmdstep_{self.dmd_steps}")
                        logger.debug(
                            "Removing echo and movie file from new directory")
                        os.remove(os.path.join(
                            f"dmdstep_{self.dmd_steps}", self.parameters["dmd params"]["Echo File"]))
                        ...
                        shutil.copy("dmdStart.pdb",
                                    f"dmdstep_{self.dmd_steps}/")

                    logger.debug(
                        f"Changing direcetory from {os.getcwd()} to dmdstep_{self.dmd_steps}")
                    ...
                    self.stop = self.timer_went_off()
                    curr_energy = dmd_simulation.get_average_potential_energy(
                        self.parameters["dmd params"]["Echo File"])
                    ...
                    if len(dmd_average_energies) >= 2:
                        logger.info(
                            f"[Delta Last Sim. ] ==>> {abs(curr_energy[0] - dmd_average_energies[-2][0]):.5f}")
                        if self.dmd_converged(dmd_average_energies):
                            logger.info("")
                            logger.info("Converence achieved!")
                            converged = True
                            logger.info(
                                f"Absolute delta in energy: {abs(curr_energy[0]-dmd_average_energies[-2][0]):.5f}")
        ...
        if self.stop:
            ...
            if converged:
                ...
                logger.info(
                    f"[Ave. Potential Energy] ==>> {min_step[0]:.5f} ({min_step[1]:.5f})")
            ...
            logger.info(
                "[RUN ##]---------[Ave Pot. Energy]---------[Est. Phys Time (ns)]---------")
            ...
            logger.info("...")
            utilities.make_movie(
                "initial.pdb", self.parameters["dmd params"]["Movie File"], "movie.pdb")
            ...
            dmd_structures_energies = [float(line[4]) for line in dmd_simulation.get_echo_data(
                self.parameters['dmd params']['Echo File'])]
            # Now we are done
            # Should really add the dmd_structures_energies to the protein objects...as well as protonation states

            for structure, energy in zip(self.dmd_structures, dmd_structures_energies):
                structure.dmd_energy = energy

            # TODO proto check for some file and then add that...
            if not self.parameters["dmd params"]["titr"]["titr on"]:
                os.remove("movie.pdb")
    ...
    def cluster(self):
        ...
        if [d for d in os.listdir() if "movie_" in d]:
            ...
            if self.cores > 1:
                ...
                indexes = [(i, j) for i, j in itertools.product(
                    range(0, len(self.dmd_structures)), range(0, len(self.dmd_structures)))]
                ...
                result = np.ctypeslib.as_ctypes(
                    np.zeros((len(self.dmd_structures), len(self.dmd_structures))))
                shared_array = sharedctypes.RawArray(result._type_, result)

                def computeRMSD(vals):
                    tmp = np.ctypeslib.as_array(shared_array)
                    for i, j in vals:
                        tmp[i, j] = self.dmd_structures[i].aa_rmsd(
                            self.dmd_structures[j])
                        tmp[j, i] = tmp[i, j]

                # start all the procs
                procs = [Process(target=computeRMSD, args=(vals,))
                         for vals in chunked_index]
                ...
                distance_matrix = np.zeros(
                    [len(self.dmd_structures), len(self.dmd_structures)], dtype=float)
                ...
                for row, first_structure in enumerate(self.dmd_structures):
                    for col, second_structure in enumerate(self.dmd_structures[row+1:]):
                        distance_matrix[row][col] = first_structure.aa_rmsd(
                            second_structure)
                        distance_matrix[col][row] = distance_matrix[row][col]

            end = timer()
            logger.info(
                f"Time elapsed during RMSD calculation: {datetime.timedelta(seconds = int(end -start))}")
            ...
            start = timer()
            output = hdbscan.HDBSCAN(
                metric='precomputed', core_dist_n_jobs=self.cores, min_cluster_size=50, min_samples=25)
            ...
            logger.info(
                f"Time elapsed during HDBSCAN clustering: {datetime.timedelta(seconds = int(end -start))}")
            ...
            if num_clusters > self.parameters["Max Clusters"] or num_clusters <= 0:
                ...
                logger.info(">>>> Cluster Parameters >>>>")
                logger.info(
                    f"[Max Num. Clusters] ==>> {self.parameters['Max Clusters']}")
                logger.info(
                    "[Cluster Method]    ==>> Single Linkage Heirarchy")
                logger.info("[Cluster Criterion] ==>> Max Cluster")
                logger.info("...")
                start = timer()
                y = [distance_matrix[j][i] for j in range(
                    len(distance_matrix)) for i in range(j+1, len(distance_matrix))]
                Z = scipy.cluster.hierarchy.linkage(y)
                cluster_labels = scipy.cluster.hierarchy.fcluster(
                    Z, t=self.parameters['Max Clusters'], criterion='maxclust')
                end = timer()

                num_clusters = len(set(cluster_labels))
                logger.info(f"[Num. of Clusters]  ==>> {num_clusters}")
                logger.info(
                    f"Time elapsed during Single Linkage clustering: {datetime.timedelta(seconds = int(end -start))}")

            else:
                cluster_labels = output.labels_

            logger.info(
                f"[Minimum Energy]    ==>> {'true' if self.parameters['Cluster Energy'] else 'false'}")
            logger.info(
                f"[Cluster Centroid]  ==>> {'true' if self.parameters['Cluster Centroid'] else 'false'}")
            ...
            if self.parameters["Cluster Energy"]:
                for index in self.dmd_structures.keys():
                    min_structures.append(
                        min(self.dmd_structures[index], key=lambda i: i.dmd_energy))
            ...
            if self.parameters["Cluster Centroid"]:
                for index in self.dmd_structures.keys():
                    ...
                    sub_distance_matrix = np.zeros(
                        (len(self.dmd_structures[index]), len(self.dmd_structures[index])))
                    ...
                    for row, first_structure in enumerate(self.dmd_structures[index]):
                        first_structure_index = old_dmd_structures.index(
                            first_structure)
                        for col, second_structure in enumerate(self.dmd_structures[index][row+1:]):
                            second_structure_index = old_dmd_structures.index(
                                second_structure)
                    ...
                    with_rows = list(
                        zip(self.dmd_structures[index], rows_sums))
            ...
            if not self.sp_PDB_structures:
                logger.info(
                    "No clustering method specified, using minimum energy from each cluster")
                for index in self.dmd_structures.keys():
                    min_structures.append(
                        min(self.dmd_structures[index], key=lambda i: i.dmd_energy))
        ...
        for struct in self.sp_PDB_structures:
            out = f"[{struct.name}]            {struct.dmd_energy:.5f}"
    ...
    def qm_singlepoints(self):
        ...
        for index, struct in enumerate(self.sp_PDB_structures):
            ...
            logger.debug(
                f"Changing directory from {os.getcwd()} to sp_{struct.name}")
            ...
            if os.path.isfile("energy"):
                ...
                if i >= 2:
                    struct.qm_energy = qm_calculation.TMcalculation.get_energy(
                        cycle=1)
                    logger.info(
                        f"[QM Energy]       ==>> {struct.qm_energy:.5f} Hart (finished)")
                    os.chdir("../")
                    continue
            ...
            if not os.path.isfile("control"):
                ...
                try:
                    sj = setupTMjob(
                        parameters=self.parameters["qm params"], timeout=0)
                ...
                dirs = [d for d in os.listdir(
                    "../") if os.path.isdir(os.path.join("../", d)) and "sp_movie_" in d]
                    ...
                    for mo_file in constants.MO_FILES:
                        if os.path.isfile(os.path.join(sp_directory, mo_file)):
                            ...
                            shutil.copy(os.path.join(
                                sp_directory, mo_file), f"./{mo_file}")
            ...
            if os.path.isdir("trun_backup"):
                for mo_file in constants.MO_FILES:
                    if os.path.isfile(os.path.join("trun_backup", mo_file)):
                        logger.debug(
                            f"Copying up {mo_file} file from trun_backup")
                        shutil.copy(os.path.join(
                            "trun_backup", mo_file), f"./{mo_file}")

            qm_params = self.parameters["qm params"].copy()
            qm_params["calculation"] = 'sp'
            start = timer()
            sp = qm_calculation.TMcalculation(
                self.cores, parameters=qm_params, run_dir=self.scratch, time=self.controller.time_left())
            end = timer()
            self.stop = self.timer_went_off()
            ...
            try:
                ...
                logger.error(
                    f"Singlepoint could not converge in {qm_params['scf']['iter']*2} scf cycles")
                logger.error(
                    "Structure may be very weird, continuing to next structure")
                struct.qm_energy = 0.0

            logger.info(f"[QM Energy] ==>> {struct.qm_energy:.5f} Hart")
            logger.info(
                f"Time elapsed during QM SP calculation: {datetime.timedelta(seconds = int(end -start))}")
            logger.debug(
                f"Changing directory from {os.getcwd()} to {os.path.abspath('..')}")
            os.chdir("../")

            if self.stop:
                logger.debug(
                    "Stop received while doing singlepoint calculations")
                return
        ...
        for struct in self.sp_PDB_structures:
            out_string = f"[{struct.name}]         {struct.dmd_energy:0<12}                {struct.qm_energy:0<14.6f}            {struct.score:.3f}"
            if struct == winning_struct:
                out_string += "      --w---w--"

            logger.info(out_string)

        logger.info("")
        logger.info(f"[WINNER]     ==>> {winning_struct.name}")
        logger.info(
            f"[QM  Energy] ==>> {winning_struct.qm_energy:.5f} Hartrees")
        logger.info(
            f"[DMD Energy] ==>> {winning_struct.dmd_energy:.5f} kcal/mol")
    ...
    def qm_optimization(self):
        ...
        if os.path.isfile("energy"):
            ...
            if total_cycles >= qm_params["geo_iterations"]:
                logger.info(
                    f"Previous run completed max number of cycles: {total_cycles}")
                finished = True

            else:
                logger.info(
                    f"Restarting geometry optimization from cycle: {total_cycles}")
        ...
        if not finished:
            ...
            if not os.path.isfile("coord"):
                logger.debug("Creating coord file")
                dmd_to_qm.protein_to_coord(
                    self.pdb_winner, self.parameters["QM Chop"])
            ...
            if not os.path.isfile("control"):
                ...
                possible_sp_dir = "../sp_" + self.pdb_winner.name
                found_mos = False
                if os.path.isdir(possible_sp_dir):
                    for mo_file in constants.MO_FILES:
                        if os.path.isfile(os.path.join(possible_sp_dir, mo_file)):
                            ...
                            found_mos = True
                            shutil.copy(os.path.join(
                                possible_sp_dir, mo_file), f"./{mo_file}")

                else:
                    # Check for any of these directories actually...
                    dirs = [d for d in os.listdir(
                        "../") if os.path.isdir(os.path.join("../", d)) and "sp_movie_" in d]
                    dirs.sort(reverse=True, key=lambda i: int(
                        i.split("_")[-1]))
            ...
            start = timer()
            geo = qm_calculation.TMcalculation(
                self.cores, parameters=qm_params, time=self.controller.time_left(), run_dir=self.scratch)
            end = timer()
            logger.info(
                f"Time elapsed during QM Opt calculation: {datetime.timedelta(seconds = int(end -start))}")
            ...
            if geo.scfiterfail():
                ...
                logger.info(
                    f"Failed to converge in a total of {total_cycles} geometry cycles")
        ...
        if self.stop:
            ...
            self.pdb_winner.qm_energy = qm_calculation.TMcalculation.get_energy()
            logger.info("")
            logger.info(f"[Final QM Energy] ==>> {self.pdb_winner.qm_energy}")

        # Now we reinstall the coords into the protein!
        self.to_next_iteration = dmd_to_qm.coord_to_protein(
            self.pdb_winner, self.parameters["QM Chop"])
        ...
        if qm_params['geo_iterations'] < total_cycles and self.parameters["qm params"]["calculation"] == "forceopt" and not os.path.isfile("Optimization/GEO_OPT_CONVERGED"):
            ...
            submitturbomole.main(
                _cores=self.cores, _time=self.controller._time, _nodes=1, _sub=True)
            os.chdir("..")
    ...
    def finish_iteration(self):
        ...
        if os.path.isfile(os.path.join(self.root_directory, "phd_energy")):
            ...
            first_line = (
                "[Iteration]        [QM Energy (Hart)]        [DMD Energy (kcal)]\n")

        outfile_lines.append(
            f"{self.iter_number:0>2d}                 {self.pdb_winner.qm_energy:.5f}                 {self.pdb_winner.dmd_energy:.5f}\n")
        ...
        if os.path.isfile(f"dmd/{self.parameters['dmd params']['Movie File']}"):
            logger.info(
                f"[Compressing] ==>> {self.parameters['dmd params']['Movie File']}")
            with tarfile.open(f"dmd/{self.parameters['dmd params']['Movie File']}.tar.gz", "w:gz") as tar:
                tar.add(f"dmd/{self.parameters['dmd params']['Movie File']}", arcname=os.path.basename(
                    f"dmd/{self.parameters['dmd params']['Movie File']}"))

            logger.info(
                f"[Removing] ==>> {self.parameters['dmd params']['Movie File']}")
        ...
        if "Default MOs" in self.parameters.keys() and self.parameters["Default MOs"] is not None:
            for mo_file in constants.MO_FILES:
                if os.path.isfile(os.path.join(self.parameters["Default MOs"], mo_file)):
                    logger.info(
                        f"Copying over {mo_file} file (from Default MOs {self.parameters['Default MOs']})")
                    shutil.copy(os.path.join(
                        self.parameters["Default MOs"], mo_file), f"./{mo_file}")
