...
def main(_cores: int=None, _time: int=None, _nodes: int=1, _sub: bool=True):
    ...
    if cores is None or time is None:
        ...
        parser = argparse.ArgumentParser(description="Submits a TURBOMOLE job to a queuing system")
        parser.add_argument("-N", dest="nodes", type=int, required=False, default=1,
                            help=f"The number of nodes to run the TURBOMOLE job on.")
        parser.add_argument("-n", dest="cores", type=int, required=True,
                            help=f"The number of cores per node to run the TURBOMOLE job with.")
    ...
    job_params = {
        ...
        "run_script": os.path.join(os.path.dirname(__file__), "runturbomole.py"),
        "time": time
    }
